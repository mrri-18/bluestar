...
int total(int* money, int* b) {   //포인터와 변수를 매개변수로 전달
	for (int i = 0; i < 4; i++)  // 배열의 주소에 접근해 값 반복해서 더하기
		*money = *(b + i) +*money;
	return *money;
...
int main() {
	...
	cout << "현재 SWING 자산관리 서비스에 등록된 회원님의 총 자산은 " << total(&total_money, q) << "원 입니다." << endl;   //총 금액 출력 
	cout << "=======================================================================" << endl;
}
